"""Calibration & rigorous threshold selection on real OOD scores.

    python examples/calibration_analysis.py

Shows, on synthetic-but-real kNN OOD distances:
  1. a conformal cut that replaces the eyeballed p90 with an FPR guarantee,
  2. risk-coverage / AURC over the whole confidence ordering,
  3. cost- vs constraint- vs Youden-based operating points,
...
# Realistic drift: half "camouflaged" (drawn like inliers -> unavoidable misses)
# and half clearly drifted. The overlap makes the operating points genuinely
# differ, so the cost asymmetry actually moves the threshold.
...
# Selective prediction keys off real correctness, which the system posits is
# monotonic in OOD distance: accuracy falls smoothly as samples drift out.
p_correct = 1.0 / (1.0 + np.exp((prod_dist - 0.045) / 0.008))
correct = rng.random(prod_dist.size) < p_correct
confidence = -prod_dist  # nearer the core = more confident
...
# --- 4. Bootstrap CI -> CI-gated threshold adjustment ---------------------
point, lo, hi = conformal_threshold_ci(cal_dist, alpha, n_boot=500, seed=2)
deployed = 0.0335  # an old, now-stale deployed cut
